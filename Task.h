...
public:
    explicit Task(int id, const QString &taskName, const QString &taskDescription,
                  const QString &dateDeadline, const QString taskImportance, QObject *parent = nullptr)
        : QObject(parent), m_id(id), m_taskName(taskName), m_taskDescription(taskDescription),
        m_dateDeadline(dateDeadline), m_taskImportance(taskImportance) {}
